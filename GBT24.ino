...
void canRead();
void computeAndSetCRC(uint8_t* data, uint8_t data_id_low, uint8_t length);
...
/* ---------------- TIME ---------------- */
int startYear = 2026, startMonth = 2, startDay = 26;
...
void loop() {
  ...
  if (systemActive) {
    ...
    if (hvReq == 0x00 && now - startupTime >= DISCONNECT_PHASE) {
      hvReq = 0x20;
      Serial.println("HV REQUEST CONNECT");
    }
...
void handleVoltageRequest() {
  ...
  if (lastState == HIGH && currentState == LOW) {
    ...
    if (voltageValid) {
      ...
      Serial.println(latestPackVoltage);
    }
    else {
      Serial.println("Voltage not yet received");
    }
...
void sendSftyInfo() {
  ...
  computeAndSetCRC(frame.data.byte, ID_SFTYINFO & 0xFF, 3);
...
void sendCtlCmd() {
  ...
  computeAndSetCRC(frame.data.byte, ID_CTLCMD & 0xFF, 4);
...
void sendVehInfo() {
  ...
  computeAndSetCRC(frame.data.byte, ID_VEHINFO & 0xFF, 6);
...
void sendVehTi() {
  ...
  computeAndSetCRC(frame.data.byte, ID_VEHTI & 0xFF, 6);
...
void computeAndSetCRC(uint8_t* data, uint8_t data_id_low, uint8_t length) {
  ...
  data[0] = CRC8_SAE_J1850_ZERO::calculate(crc_data, length);
